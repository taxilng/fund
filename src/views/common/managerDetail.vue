        ...
        <div>
          ...
          <div class="table-row">
            <table>
              ...
              <tbody>
                <tr v-for="(el, index) in managerList" :key="index">
                  <td>{{ el.FEMPDATE }}</td>
          ...
          <h5>现任基金经理简介</h5>
          <div v-for="(el, index) in managerDetail" :key="index">
...
export default {
  ...
  methods: {
    ...
    getManagerList(code) {
      let url = `/FundMApi/FundManagerList.ashx?FCODE=${code}&deviceid=Wap&plat=Wap&product=EFund&version=2.0.0&Uid=&_=${new Date().getTime()}`;
      this.$axios.get(url).then((res) => {
        this.managerList = res.Datas;
      });
    },
    getMangerDetail(code) {
      let url = `/FundMApi/FundMangerDetail.ashx?FCODE=${code}&deviceid=Wap&plat=Wap&product=EFund&version=2.0.0&Uid=&_=${new Date().getTime()}`;
      this.$axios.get(url).then((res) => {
        this.managerDetail = res.Datas;
      });
    },
